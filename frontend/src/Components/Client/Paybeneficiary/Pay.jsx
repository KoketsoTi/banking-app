...
import { Benefciary } from '../../../Data/mockedData';
    ...
    return (
        <Box className="Box">
            ...
            <Box display="flex" justifyContent="space-between" alignItems="center">
                <Box mb="30px">
                    <Typography variant="h5" fontWeight="bold" style={{color: "#141b2d"}} sx={{ m: "0 0 5px 0" }}>Previously added beneficiaries</Typography>
                </Box>
            </Box>
            {/* CONTENT */}
            <Box>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3  gap-2 lg:xl:gap-4">
                    {Benefciary.map((pay) => {
                        return (
                            <div className="card lg:xl:w-96 bg-base-100 shadow-xl cursor-pointer" key={pay.id}  onClick={() => payUser(pay)} >
                                        ...
                                        <div className="avatar placeholder">
                                            <div className="bg-neutral-focus text-neutral-content rounded-full w-16 lg:xl:w-20">
                                                <span className="text-2xl lg:xl:text-3xl">
                                                    {pay.fname.slice(0, 1)?.toUpperCase()}
                                                </span>
                                            </div>
                                        </div>
                                        <div className="ml-5 mt-1">
                                            <div className="text-sm lg:xl:text-lg text-start">{pay.fname} </div>
                                            <div className="text-xs lg:xl:text-base text-start">Last paid: {pay.date}</div>
                                            <div className="text-xs lg:xl:text-base text-start">R{pay.amount.toLocaleString()}</div>
                                        </div>
                                    </div>    
                                </div>
                            </div>
                ...
                <label htmlFor="my-modal-4" className="btn btn-sm btn-circle absolute text-black hover:text-white right-2 top-2">✕</label>
                    ...
                    <form >
                        ...
                        <div className="form-group text-start pay-button col mt-10">
                            <button  onClick={handleSubmit(onSubmit)} className="rounded-none relative w-full lg:xl:w-36 flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">Add Beneficiary</button>
                        </div>           
                    </form>
                </label>
            </label>
